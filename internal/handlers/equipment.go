...
func (h *EquipmentHandler) GetEquipmentByDeviceID(w http.ResponseWriter, r *http.Request) {
	...
	if err == sql.ErrNoRows {
		helpers.JsonResponseError(w, http.StatusBadRequest, "device id is not in database", "GET /api/v1/device/{id}")
		return
	} else if err != nil {
	...
	if e == nil {
		helpers.JsonResponseError(w, http.StatusNotFound, "device id is not in database", "GET /api/v1/device/{id}")
		return
	}
